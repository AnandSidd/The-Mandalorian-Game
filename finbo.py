...
class Board:
	...
	def stick_rider_bullets(self):
		...
		# self._old_bullet_pos.append(pos)
	# def bullet_collision(self,y,x):
	# 	cr=self.get_px(y,x)
	# 	if cr=="\u2580" or cr=="\u2588":
	# 		return 1
	# 	else:
			...
			if((x[1])<2300):
				...
				if(rtc=="\u2588"):
					...
					if self._grid[x[0]][x[1]]=="\u2580":
						...
						self._bullet_len[i]=460
				# elif rtc=="O":
				# 	if(self._bullet_pos[i][1]>=0):
				# 		self._grid[x[0]][x[1]]="O"
				...
				i+=1
					...
					# print("YES",end="")
				# if(self._grid[x[0]][x[1]]!=" "):
					# print(self._grid[x[0]][x[1]],end="")
				# rtc=self.bullet_collision(x[0],x[1])
				# print(rtc)
				# if rtc==1:
					# self._grid[x[0]][x[1]]="*"
				# else:
				# self._bullet_len[i]+=2
	# def delete_bullets(self,manx,x):
	# 	# i=0:
	# 	st=(manx+x-238)
	# 	if st<0:
	# 		st=0
	# 	for y in range(st,manx+x):
	# 		for z in range(2,47):
	# 			if self._grid[z][y]=="-":
	# 				self._grid[z][y]=" "

		# for x in self._old_bullet_pos:
		# 	self._grid[x[0]][x[1]]=" "
		# 	self._old_bullet_pos[i][1]=self._bullet_pos[i][1]
		# 	i+=1
...
if(flag==0 and flag1==0):
	while True:
		...
		if chbuff != 'e':
			...
			print("\033[54;0f",end="")
			break
		# obj3.printBoard(manx,many)
	# while True:
    	# chbuff=input_to(obj4)
		# 	obj3.printBoard(manx+1,many)
		# else:
    # print(chbuff)